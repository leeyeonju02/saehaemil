...
export default function GalleryAlbumAdminBar({ albumId }: Props) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/gallery/${encodeURIComponent(albumId)}`, {
        method: "DELETE",
  ...
  return (
    <>
      ...
      >
        <Button
          component={Link}
          href={`/gallery/new?edit=${encodeURIComponent(albumId)}`}
          variant="outlined"
          size="medium"
        >
          수정
      ...
      <Dialog open={deleteOpen} onClose={() => !deleting && setDeleteOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
            이 앨범을 데이터베이스에서 삭제합니다. 되돌릴 수 없습니다. 계속하시겠습니까?
          </Typography>
          {errorText ? (
